...
describe('Make Decision', function () {
    ...
    it('Complaint which allows for prescription', function () {
        var decisions = decision.getDecision(new Encounter().setObservation("Complaint", ["Cold"]).setGender("Male").setAge(25).setObservation("Weight", 40));
    ...
    it('Do not give any medicine for chloroquin resistant malaria to women between 16-40', function () {
        var decisions = decision.getDecision(new Encounter().setObservation("Complaint", ["Chloroquine Resistant Malaria"]).setGender("Female").setAge(25).setObservation("Weight", 40));
    ...
    it('Provide day wise instructions when specified for days separately', function () {
        var decisions = decision.getDecision(new Encounter().setObservation("Complaint", ["Fever"]).setObservation("Paracheck", ["Positive for PF"]).setGender("Male").setAge(25).setObservation("Weight", 40));
    ...
    it('Do not provide day wise instructions when not specified for days separately', function () {
        var decisions = decision.getDecision(new Encounter().setObservation("Complaint", ["Cough"]).setGender("Male").setAge(25).setObservation("Weight", 40));
    ...
    it('Print special instruction', function () {
        var decisions = decision.getDecision(new Encounter().setObservation("Complaint", ["Ring Worm"]).setGender("Male").setAge(25).setObservation("Weight", 40));
    ...
    it('In cough do not give Septran to potentially pregnant women', function () {
        ...
        decisions = decision.getDecision(new Encounter().setObservation("Complaint", ["Cough"]).setGender("Female").setAge(25).setObservation("Weight", 40));
        ...
        decisions = decision.getDecision(new Encounter().setObservation("Complaint", ["Cough"]).setGender("Female").setAge(45).setObservation("Weight", 40));
        expect((decisions[0].value.match(/सिफ्रान/g) || []).length).to.equal(0, decisions[0].value);
        expect((decisions[0].value.match(/सेप्ट्रान/g) || []).length).to.equal(1, decisions[0].value);

        decisions = decision.getDecision(new Encounter().setObservation("Complaint", ["Boils"]).setGender("Female").setAge(25).setObservation("Weight", 40));
        expect((decisions[0].value.match(/सिफ्रान/g) || []).length).to.equal(1, decisions[0].value);
        expect((decisions[0].value.match(/सेप्ट्रान/g) || []).length).to.equal(0, decisions[0].value);

        decisions = decision.getDecision(new Encounter().setObservation("Complaint", ["Wound"]).setGender("Female").setAge(25).setObservation("Weight", 40));
        expect((decisions[0].value.match(/सिफ्रान/g) || []).length).to.equal(1, decisions[0].value);
        expect((decisions[0].value.match(/सेप्ट्रान/g) || []).length).to.equal(0, decisions[0].value);
    });
    ...
    it('Before food and after food instruction', function () {
        ...
        decisions = decision.getDecision(new Encounter().setObservation("Complaint", ["Acidity"]).setGender("Male").setAge(25).setObservation("Weight", 40));
    ...
    it('Multiple complaints without same medicines', function () {
        const decisions = decision.getDecision(new Encounter().setObservation("Complaint", ["Cold", "Body Ache"]).setGender("Male").setAge(25).setObservation("Weight", 40));
    ...
    it('Multiple complaints with overlapping medicines', function () {
        var decisions = decision.getDecision(new Encounter().setObservation("Complaint", ["Cold", "Fever", "Body Ache"]).setGender("Male").setAge(25).setObservation("Weight", 40));
    ...
    it('Multiple complaints with overlapping medicines and different order of medicines', function () {
        var decisions = decision.getDecision(new Encounter().setObservation("Complaint", ["Cold", "Body Ache", "Fever"]).setObservation("Paracheck", ["Positive for PF and PV"]).setGender("Male").setAge(25).setObservation("Weight", 40));
    ...
    it('Alert should be only for the decision for the complaint', () => {
        var complaintConceptName = "Complaint";
        var decisions = decision.getDecision(new Encounter().setObservation(complaintConceptName, ["Cold", "Vomiting"]).setGender("Male").setAge(10).setObservation("Weight", 22));
        expect(decisions[0].alert).to.equal(undefined);
    ...
    it('Boundary condition of weight', () => {
        var complaintConceptName = "Complaint";
        var decisions = decision.getDecision(new Encounter().setObservation(complaintConceptName, ["Fever"]).setGender("Male").setAge(10).setObservation("Weight", 5.5).setObservation("Paracheck", ["Positive PV"]));
        expect(decisions.length).to.equal(1);
    });

    it('Give malaria medicine based on paracheck being positive', () => {
        var complaintConceptName = "Complaint";
        var decisions = decision.getDecision(new Encounter().setObservation(complaintConceptName, ["Cold"]).setGender("Male").setAge(10).setObservation("Weight", 5.5).setObservation("Paracheck", ["Positive PV"]));
    ...
    it('Give malaria medicine based on paracheck being positive - when fever is also specified', () => {
        var complaintConceptName = "Complaint";
        var decisions = decision.getDecision(new Encounter().setObservation(complaintConceptName, ["Cold", "Fever"]).setGender("Male").setAge(10).setObservation("Weight", 8).setObservation("Paracheck", ["Positive PV"]));
        expect((decisions[0].value.match(/क्लोरोक्विन/g) || []).length).to.equal(3, decisions[0].value);
    });
    
    it('Fever, Body Ache & Vomiting', () => {
        var complaintConceptName = "Complaint";
        var decisions = decision.getDecision(new Encounter().setObservation(complaintConceptName, ["Fever", "Body Ache", "Vomiting"]).setGender("Male").setAge(20).setObservation("Weight", 18));
        var message = completeValue(decisions);
